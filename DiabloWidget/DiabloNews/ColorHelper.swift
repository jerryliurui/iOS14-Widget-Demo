...
//
//  图片取色+判断图片深浅颜色类
//  图片取色逻辑来自:https://github.com/jathu/UIImageColors
//
//  Created by JerryLiu on 2020/7/21.
...
extension UIColor {
    ...
    func r() -> CGFloat {
        return self.cgColor.components?[0] ?? 0.0
    }
    ...
    func g() -> CGFloat {
        ...
        if (count == 2) {
            ...
            return self.cgColor.components?[1] ?? 0.0;
        }
    }
    ...
    func b() -> CGFloat {
        ...
        if (count == 2) {
            ...
            return self.cgColor.components?[2] ?? 0.0;
        }
    }
        
    func a() -> CGFloat {
        let count = self.cgColor.numberOfComponents;
        return self.cgColor.components?[count - 1] ?? 0.0;
    }
    
    //判断一个颜色是否暗色
    public func currentColorIsDarkColor() -> Bool? {
        //公式参考 https://www.w3.org/WAI/ER/WD-AERT/#color-contrast
        //((Red value X 299) + (Green value X 587) + (Blue value X 114)) / 1000
        let component0 = self.cgColor.components?[0] ?? 0.0
        let component1 = self.cgColor.components?[1] ?? 0.0
        let component2 = self.cgColor.components?[2] ?? 0.0
        
        let colorBrightness = ((component0 * 299) + (component1 * 587) + (component2 * 114)) / 1000
    ...
    public func fetchWidgetArticleTitleColorName() -> String? {
        let isDarkColor = self.currentColorIsDarkColor() ?? false
        if isDarkColor {
            return "articleTitleForDarkBG"
        } else {
            return "articleTitleForLightBG"
        }
    }
}
